...
var validateLDAP = require("./ldap.js");
      	...
      	if(err){
      		...
      		else if(resp.statusCode == 200)
      		{
				...
				logger.info("--- login id used for db check:"+loginId);
	      		...
	      		// Currently LDAP microservice does only authentication and returns only SUCCESS / FAILURE
				// In future , LDAP would provide user profile as response in JSON.
				// For testing  and dev purpose , preloaded response with req , req is basically a email id.
	      		//response = resp;
				//var organization = process.env.organization;
				//var loginId;
				//if (organization == 'Kaiser'){
				//	loginId = req.body._id.toUpperCase();
				//} else {
				//	loginId = req.body._id;
				//}
				...
				//add the user as submitter role - temp fix
				// will insert a row only when it doesn't exist with submitter role'
...
//						//console.log( docs );
//						if (err){
//							logger.info('--- error inserting user ---:' + docs);
//						} else {
//							logger.info('--- docs inserted successfully---:' + docs);
//						}
//					}
//				); 
...
function updateTokenValidityForUser(req,callback){
	...
	logger.info("---- tokenvalidity:"+req.Tokenvalidity);
	var _id = req._id;
